...
class XunLeiTester:
    ...
    async def test_single_account(self, account):
        ...
        #     rsp = await client.get(url, data={"method": "gain"})
        ...
        async with aiohttp.ClientSession() as client:
            ...
            async with client.post(url, data=data, headers=self.headers, proxy=proxy) as rsp:
                # print(rsp.status)
                # print(rsp.cookies)
            ...
            async with client.get(url, headers=self.headers2, proxy=proxy) as rsp:
                print(rsp.status)
            ...
            vip_url = "https://xluser-ssl.xunlei.com/xlcenter/v1/" \
                      "GetAllVipInfo?" \
                      f"csrf_token={csrf_token}"
            print([{cookie.key: cookie.value} for cookie in client.cookie_jar])

            async with client.get(vip_url, headers=self.headers2, proxy=proxy) as rsp:
                vip_info = await rsp.json()

                print(vip_info)
                if vip_info.get('code', 0) == 200:
                    self._db.max(REDIS_XUNLEI_KEY, account)
                    print("ok")
                else:
                    self._db.decrease(REDIS_XUNLEI_KEY, account)
                    print("no")
    ...
    async def run(self):
        ...
        for start in range(0, xunlei_count, TEST_COUNT):
            ...
            await Multi(map(self.test_single_account, accounts))
